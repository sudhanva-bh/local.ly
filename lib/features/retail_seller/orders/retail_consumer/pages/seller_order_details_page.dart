...
class SellerOrderDetailsScreen extends ConsumerWidget {
  final OrderModel initialOrder; // Renamed to imply it's the starting point
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // ⚡ REALTIME LISTENER
    // We listen to the specific stream for this Order ID.
    final orderAsync = ref.watch(sellerOrderDetailsProvider(initialOrder.id));

    // 💡 SMART DATA: 
    // If the stream has data, use it. If it's loading/error, fallback to initialOrder.
    // This prevents the screen from flashing a loading spinner when you first open it.
    final OrderModel order = orderAsync.value ?? initialOrder;

    // 🔑 QR Data
    final String? retailerUID = ref.watch(currentUserProfileProvider).value?.uid;
    final String qrData = "${order.id}|$retailerUID";
    ...
    return Scaffold(
      ...
      bottomNavigationBar: _buildManagementBar(
        context,
        ref,
        order, // Passing the REALTIME order object here
        qrData,
      ),
    );
  }
            ...
            if (order.status == OrderStatus.pending) ...[
              ...
              Expanded(
                child: FilledButton.icon(
                  onPressed: () async {
                    // 1. Call Service
                    final result = await ref.read(orderServiceProvider).receiveOrder(order.id);
          ...
          return pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.SizedBox(height: 5),
                pw.Center(
                  child: pw.Text(order.id, style: const pw.TextStyle(fontSize: 8)),
                ),
              ],
            ),
          );
