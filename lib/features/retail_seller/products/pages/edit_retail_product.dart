...
// You'll need a RetailNavPage or similar
// import 'package:locally/features/retail_seller/retail_nav_page.dart'; 

class EditRetailProduct extends ConsumerStatefulWidget {
  final String productId;
  const EditRetailProduct({super.key, required this.productId});

  @override
  ConsumerState<EditRetailProduct> createState() => _EditRetailProductState();
}

class _EditRetailProductState extends ConsumerState<EditRetailProduct> {
  final _formKey = GlobalKey<FormState>();

  late TextEditingController nameCtrl;
  late TextEditingController descCtrl;
  late TextEditingController priceCtrl;
  late TextEditingController discountedPriceCtrl; // 👈 Added
  ...
  Widget build(BuildContext context) {
    ...
    final productAsync = ref.watch(
      retailProductByIdProvider(widget.productId),
    );
    ...
    return Scaffold(
      ...
      body: productAsync.when(
        data: (product) {
          ...
          if (!_initialized) {
            nameCtrl = TextEditingController(text: product.name); // 👈 Use retail field
            descCtrl = TextEditingController(text: product.description);
            priceCtrl = TextEditingController(
              text: product.price.toStringAsFixed(2),
            );
            // 👈 Added
            discountedPriceCtrl = TextEditingController(
            ...
            _selectedCategory = product.category; // 👈 Already an enum
          ...
          return Form(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  /// 📸 Image Section (Identical, but uses ImagePickerGrid)
                  Padding(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: _buildField(
                                "Discounted Price (Optional)", // 👈 Added
                                discountedPriceCtrl,
                                text,
                                colors,
                                keyboard: TextInputType.number,
  ...
  Future<void> _saveProduct(
    ...
    RetailProduct original, // 👈 Updated model
  ) async {
    ...
    // 👈 Use retail service
    final productService = ref.read(retailProductServiceProvider);
  ...
  Future<void> _deleteProduct(
    BuildContext context,
    RetailProduct product, // 👈 Updated model
  ) async {
    ...
    try {
      ...
      // 👈 Invalidate retail provider
      ref.invalidate(userRetailProductsProvider); 
      ...
      // TODO: Navigate to the correct retail page
      // Navigator.pushReplacement(
      //   context,
      //   MaterialPageRoute(
      //     builder: (context) => RetailNavPage(
      //       initialIndex: 1, // Or wherever your products page is
      //     ),
      //   ),
      // );
       Navigator.pop(context); // Pop back to view page
       Navigator.pop(context); // Pop back to list page
