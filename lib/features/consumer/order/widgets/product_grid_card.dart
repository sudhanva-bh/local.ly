...
/// Helper provider to fetch Seller Name
...
class RetailProductGridCard extends ConsumerStatefulWidget {
  final RetailProduct product;

  const RetailProductGridCard({super.key, required this.product});

  @override
  ConsumerState<RetailProductGridCard> createState() =>
      _RetailProductGridCardState();
}

class _RetailProductGridCardState extends ConsumerState<RetailProductGridCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final isOOS = widget.product.isOutOfStock; // Access helper from model
        ...
        child: Container(
          ...
          child: ClipRRect(
            ...
            child: Column(
              ...
              children: [
                // -------------------------------------------------------------
                // IMAGE SECTION (With Out of Stock Logic)
                // -------------------------------------------------------------
                Expanded(
                  child: Stack(
                    fit: StackFit.expand,
                    children: [
                      ...
                      Hero(
                        ...
                        child: widget.product.imageUrls.isNotEmpty
                            ? _buildImage(isOOS) // Extracted helper below
                            : Container(
                                color: context.colors.surfaceContainerHigh,
                              ),
                      ),

                      // 2. Out of Stock Overlay (New)
                      if (isOOS)
                        Positioned.fill(
                          child: Container(
                            decoration: BoxDecoration(
                      ...
                      // 3. Distance Badge (Hide if OOS to reduce clutter, or keep it)
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          const Spacer(),
                          // Hide rating if OOS? Or keep it.
                          // Keeping it usually helps users wishlist items.
                          Icon(
                            Icons.star_rounded,
                            size: 14,
  ...
  // Helper to apply Grayscale if OOS
  Widget _buildImage(bool isOOS) {
    ...
    if (isOOS) {
      return ColorFiltered(
        colorFilter: const ColorFilter.mode(
          Colors.grey,
          BlendMode.saturation, // Removes color
        ),
        child: image,
      );
    }
