...
import 'package:locally/features/wholesale_seller/orders/widgets/wholesale_order_card.dart'; // Specific card

class WholesaleOrdersPage extends ConsumerWidget {
  const WholesaleOrdersPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final ordersAsync = ref.watch(currentWholesaleOrdersProvider);
            ...
            child: ordersAsync.when(
              data: (orders) {
                final filtered = orders.where((o) {
                  final matchesQuery = o.orderId.toLowerCase().contains(
                    query.toLowerCase(),
                  );
                  final matchesStatus =
                      selectedStatus == null ||
